 ...
 */
export function CrewGrid({ players, capacity, hostId, youId, onRemove }: Props) {
   ...
   return (
      <ul className='grid grid-cols-2 gap-2'>
         {players.map((player, i) => (
            <li
               key={player.id}
               className='flex min-h-[44px] items-center gap-2 rounded-xl border border-[color:var(--color-surface-border)] bg-[color:var(--color-abyss-900)]/60 px-2.5'
            >
